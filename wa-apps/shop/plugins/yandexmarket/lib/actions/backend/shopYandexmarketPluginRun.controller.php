...
SQL;
                ...
                if (!empty($this->data['export']['sku'])) {
                    ...
                    foreach ($skus as $sku) {
                        ...
                        if ($check_available && $check_sku_price && $check_count) {
                            if (count($skus) == 1) {
                                $product['price'] = $sku['price'];
                                //Самойлов НАЧАЛО 09.07.17 14:13 #
                                $product['purchase_price'] = $sku['purchase_price'];
                                //Самойлов КОНЕЦ 09.07.17 14:13
                                $product['compare_price'] = $sku['compare_price'];
                                $product['file_name'] = $sku['file_name'];
                                $product['sku'] = $sku['sku'];
                                $product['count'] = $sku['count'];
    ...
    private function addOffer($product, $sku = null)
    {
        ...
        //Самойлов НАЧАЛО 09.07.17 14:34 #
        if(isset($product['purchase_price']))
        {
            $offer['purchase_price'] = $this->format('price', $product['purchase_price']);
            $map['purchase_price'] = array(
                        'attribute' => false,
                        'callback'  => false,
                    );
        }
        //Самойлов КОНЕЦ 09.07.17 14:34
        if ($offer) {
